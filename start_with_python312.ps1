# 使用Python 3.12启动博客的PowerShell脚本
param(
    [string]$PythonPath = "E:\python\python.exe"
)

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "        wswldcs的个人博客启动脚本" -ForegroundColor Cyan
Write-Host "        使用Python 3.12" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 检查指定的Python是否存在
if (-not (Test-Path $PythonPath)) {
    Write-Host "✗ 错误: 未找到Python 3.12，路径: $PythonPath" -ForegroundColor Red
    Write-Host "请检查Python 3.12的安装路径" -ForegroundColor Yellow
    Read-Host "按任意键退出"
    exit 1
}
...
try {
    ...
    Write-Host "✗ 错误: 无法运行Python" -ForegroundColor Red
    Read-Host "按任意键退出"
    exit 1
}
...
$venvPython = "venv312\Scripts\python.exe"
$venvPip = "venv312\Scripts\pip.exe"
